...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def eval_scene(self, scene_id, dump_folder, TOP_K = 50, return_list = False,vis = False, max_width = 0.1, last_ann_id = 256):
        '''
        **Input:**
        ...
        **Output:**
        ...
        for ann_id in range(256):
            ...
            print('\r\nMean Accuracy for scene:%04d ann:%04d = %.3f' % (scene_id, ann_id, 100.0 * np.mean(grasp_accuracy[:,:])))
            scene_accuracy.append(grasp_accuracy)
        if not return_list:
    ...
    def eval_annotations(self, scene_id, ann_ids, dump_folder, TOP_K = 50, return_list = False, vis = False, max_width = 0.1):
        '''
        **Input:**
